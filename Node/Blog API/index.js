...
app.get("/random", (req, res) => {
  const randomJokes = jokes[Math.floor(Math.random() * jokes.length)];
  // console.log(randomJokes);
  return res.json(randomJokes);
});
...
app.get("/filter", (req, res) => {
  ...
  if (filteredActivies.length !== 0) {
    return res.status(200).json(filteredActivies);
  } else {
    return res.status(404).json({
      error: `Joke type does not exist`,
    });
  }
});
